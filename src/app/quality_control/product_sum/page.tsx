...
export default function ProductSumTable() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editing ? form : {
        nickel_grade: form.nickel_grade,
  ...
  const handleEdit = (user: User) => {
    try {
      const safeUser = {
        id: user.id || 0,
        nickel_grade: user.nickel_grade || "",
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure?')) {
      setLoading(true);
      try {
        const response = await fetch(`/api/product_sum?id=${id}`, {
          method: 'DELETE'
        });
  ...
  const applyFilters = (search: string) => {
    ...
    const filtered = users.filter(user => {
      ...
      try {
        if (user.nickel_grade && user.nickel_grade.toString().indexOf(search) >= 0) matchesSearch = true;
        if (user.tonnage_wmt && user.tonnage_wmt.toString().indexOf(search) >= 0) matchesSearch = true;
      } catch (e) {
        // ignore error
      }
            ...
            <div className="crud-header">
                ...
                <div className="table-container">
                    <table className="stylish-table" style={{border: '1px solid #ddd', borderCollapse: 'collapse'}}>
                      <thead>
                          <tr>
                            <th style={{textAlign: 'center', border: '1px solid #ddd', padding: '8px'}}>Nickel Grade</th>
                      ...
                      <tbody>
                          ...
                          filteredUsers.map((user) => (
                              <tr key={user.id}>
                                <td style={{border: '1px solid #ddd', padding: '8px'}}>{user.nickel_grade}</td>
                            ...
                            <form onSubmit={handleSubmit} className="modal-form">
                              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px'}}>
                                <div className="form-group">
                                    <label>Nickel Grade</label>
                                    <input
                                    type="text"
                                    value={form.nickel_grade}
                                    onChange={(e) => setForm({ ...form, nickel_grade: e.target.value })}
                                    required
                                    />
                                </div>
                                <div className="form-group">
